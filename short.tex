...
\subsubsection{Product Approach}
	Like in ~\cite{Utiyama:07}, we use the following equations to calculate the phrase scores in the triangulated translation model. 
	
	\begin{equation*}
        p_w(t \mid s) = \sum_{i} p_w(t \mid i) p_w(i \mid s)
	\end{equation*}

	\begin{equation*}
        p_w(s \mid t) = \sum_{i} p_w(s \mid i) p_w(i \mid t)
	\end{equation*}

	We are marginalizing over the pivot phrases, essentially making an independence assumption of the following form, as in~\cite{Cohn:07} :  
...
\label{sec:interpolation}
	...
	\begin{algorithm}
		...
		\begin{algorithmic}[l]
			...
			\begin{comment}
			\FOR{src-tgt in p$_{d}$} 
			\IF{src-tgt in p$_{t}$}
					
					\STATE{p$_{interp}$ =$\sum\limits_f$ $\lambda_{d}$$p_{d}$\emph{f} + $\lambda_{t}$$p_{t}$\emph{f}}
			\ENDIF
			\IF{src-tgt not in p$_{t}$} 
				\STATE{p$_{interp}$ = $\sum\limits_f$ $\lambda_{d}$$p_{d}$\emph{f}}
				%\STATE{p$_{interp}$ = $\lambda_{d}$$p_{d}$}
			\ENDIF
			\ENDFOR
			\FOR{src-tgt in p$_{t}$ and not in p$_{d}$}
				\STATE{p$_{interp}$ = $\sum\limits_f$ $\lambda_{t}$$p_{t}$\emph{f}}
			\ENDFOR
			\end{comment}
...
\label{sec:results}
	...
	\subsection{Significance Tests}
		\begin{table*}
			...
			\caption{Significance p-values: Our best compared to our baseline, and Uniform is the system with same weight for both baseline \& triangulated. All use the same tuning and heldout set}
		\end{table*}
...
\label{sec:related_work}
	...
	To our best knowledge, Malagasy, Maninkakan and Mawukakan have not been studied before in the SMT literature. Although 9 systems participated in the workshop task on Haitian-Creole, the approach of triangulation via French was not used.
\end{comment}
\section{Conclusion}

In this paper we have shown that triangulation can be an effective method to use a pivot language to inject additional training data for a low-resource language pair. This was shown in a realistic data settings with real-world low resource languages and impoverished pivot language resources for these low-resource languages. In all cases, triangulation combined with the direct system helped improve translation quality. Our experiments show that paying attention to domain adaptation issues between the original language pair and the pivot language resources helped substantially.
